...
1. **A live CRM connector: probe, don't ask.** Before asking the user anything, inspect the session's available tools for CRM-shaped connectors (tool names carrying hubspot, salesforce, pipedrive, zoho, or similar). For anything found, run one cheap read-only probe (list a single contact or company). Then tell the user what you found, plainly: "You have a HubSpot connector wired and working (probe returned a contact). Import from there?" On an auth or permission error, report the exact error and offer to skip, or pause while they fix the credential; two failed probes on the same connector means move on, never loop. Only if nothing is detected do you ask which CRM the relationships live in, and then check the connector registry for it and walk the user through connecting before falling back to a CSV export. Probes are read-only by design; never call a write or send tool during detection.
