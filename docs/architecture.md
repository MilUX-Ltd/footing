...
Footing ships with a multi-agent stack pattern. The structure carries clear write boundaries between research, curation, and execution, with explicit handoffs between them. You bring your own agents and plug them into the structure.

The pattern works directly with Anthropic's Cowork, which is where Footing is most at home. It can be adapted to other AI tools without changing the structural conventions.
...
How you wire your agents into the structure is your choice. Footing is opinionated about the boundaries. The implementation underneath is yours.
...
- **add-organisation.** Same, for an organisation, classified by relationship type (client, supplier, partner, competitor, investor, trade body), with support for organisation renames and restructures without breaking your CRM.
- **add-event.** For conferences, talks, and formal-network events. Captures the event flow including a Chatham House Rule check at intake.
- **footing-setup.** The install and onboarding skill, including the sector-research pass described above.
- **footing-update.** The update skill, described above.
- A working set of `pm-skills/` business-method reference skills (prioritisation, positioning, research synthesis, competitive analysis, and more), stripped of sector-specific framing so they apply to whatever business you run.
