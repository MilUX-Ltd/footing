...
- `docs/architecture.md`: the agent-stack section reframed as "one operator, agents with boundaries", and the four-skill v1 list replaced with the current seven-category skill set including sector-scan, daily-brief, curator, import-relationships and the ops wing.
- `.gitignore` aligned to Foothold's fuller version: recursive Obsidian per-user state exclusions (workspace, caches), Obsidian trash, and editor folders.
